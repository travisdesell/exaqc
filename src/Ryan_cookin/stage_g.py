"""Stage G: two-block circuit with enforced encoder-then-ansatz ordering.

Background: Stages E and F let evolution choose where each gate sits in
depth, so encoder and ansatz gates ended up interleaved in the champion
circuits. That's actually how data re-uploading works -- and it lifted
Stage F v2 to mean test_acc 0.904 -- but it also makes the circuits
harder to read and harder to map to the classical "feature map then
classifier" mental model.

Stage G keeps Stage F v2's universal gate vocabulary but partitions
execution into two passes:

    pass 1 (ENCODER block): every enabled gate whose method_name is
        enc_rot / enc_xx / enc_yy / enc_zz fires here, sorted by its
        evolved depth float. This is the data-dependent feature map.

    pass 2 (ANSATZ block): every other enabled gate (ry, rx, rz, cx,
        cz) fires here, sorted by depth. This is the classifier on top
        of the encoded state.

The genome is exactly Stage F v2's -- one CircuitGenome whose gates can
be either encoder or ansatz. Evolution mutates it the same way
(add_gate, qubit_swap, etc., plus grow/shrink_register). The only
difference is the qnode body iterates twice instead of once, so the
RENDERED circuit always has the encoder block as a clean prefix and
the ansatz block as a clean suffix.

Optimization is end-to-end: parameters in both blocks are torch
tensors registered on the same Adam optimizer, and gradients flow
through both passes in the backward step. So the encoder learns a
projection that makes the ansatz's job easier, and the ansatz learns
to classify what the encoder produces. The two co-train.

Run:
    PYTHONPATH=. python -m src.Ryan_cookin.stage_g \
        --datasets iris wine seeds breast_cancer \
        --seeds 0 \
        --initial_n_qubits 4 --n_genomes 120 --pop_size 25

Output:
    src/Ryan_cookin/results/stage_g.csv
    src/Ryan_cookin/results/weights_stage_g/<dataset>_seed<seed>.pt
"""
...
# Stage G reuses Stage F v2's gate-spec registration + pool building, so
# changes to the universal-gate vocabulary only need to land in one place.
from src.Ryan_cookin.stage_f import (
    _register_stage_f_specs,
    _ENC_1Q, _ENC_2Q, _ENC_ALL,
)
...
def _build_stage_g_gate_specs():
    """Same pool as Stage F v2 (full feature-dep universal gate set)."""
    _register_stage_f_specs()
...
class StageGObjective(Objective):
    """Stage G objective.

    Same shape as Stage F's objective: feature-dependent enc_* gates
    intercepted by the qnode body, ansatz gates routed through
    Gate.add_to_pennylane_circuit. The ONE difference is the order: two
    passes over gates_snapshot instead of one. Pass 1 is encoder, pass
    2 is ansatz.
    """
    ...
    def _make_qnode(self, genome: CircuitGenome, n_qubits: int):
        ...
        # Split enabled gates into encoder pass and ansatz pass at qnode-
        # construction time so the inner loop doesn't redo the lookup.
        encoder_gates = [g for g in gates_snapshot if g.enabled and g.method_name in _ENC_ALL]
        ansatz_gates = [g for g in gates_snapshot if g.enabled and g.method_name not in _ENC_ALL]
...
def train_one(
    *,
    ...
    profiler_scratch = tempfile.mkdtemp(prefix=f"exaqc_g_{dataset_name}_")
    ...
    exaqc = EXAQC(
        gate_specifications=_build_stage_g_gate_specs(),
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--n_genomes", type=int, default=120,
                   help="Matches Stage F v2 default for direct comparability.")
    p.add_argument("--pop_size", type=int, default=25)
    p.add_argument("--out", type=str, default="src/Ryan_cookin/results/stage_g.csv")
    p.add_argument("--weights_dir", type=str, default="src/Ryan_cookin/results/weights_stage_g")
