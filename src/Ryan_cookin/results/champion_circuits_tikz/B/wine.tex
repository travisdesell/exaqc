% stage B, dataset wine, seed 0
% Encoder 'reupload_euler' (D=13, N=4) applies first; not expanded here. What follows is the evolved ansatz portion of the qnode. Enabled gates: 1, test_acc=1.000.
...
\lstick{$q_{0}$} & \gate[style={fill=orange!15}]{R_y(-3.03)} & \qw \\
\lstick{$q_{1}$} & \qw & \qw \\
\lstick{$q_{2}$} & \qw & \qw \\
\lstick{$q_{3}$} & \qw & \qw
\end{quantikz}
