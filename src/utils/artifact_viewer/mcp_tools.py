...
class DashboardTools:
    ...
    def get_genome(self, run: int | str, genome_number: int) -> dict[str, Any]:
        ...
        Returns:
            ...
            parent's island (``parent_islands``), parent's species
            (``parent_species``), copy-ready ``commands`` and a
            ``dashboard_url``.
    ...
    def fitness_summary(
        ...
        Args:
            ...
            group_by: Optionally ``insert_type``, ``island``, ``species`` or
                ``generated_by``, to summarize each group separately.
        ...
        if group_by is not None and group_by not in (
            ...
            raise ValueError(
                "group_by must be insert_type, island, species or generated_by."
            )
        ...
        return {
            "single_run": {
                "genomes": (
                    ...
                    "(JSON array), crossover_type, island, species, n_gates, n_enabled_gates, "
