...
def train_value_based(genome: CircuitGenome, *, spec: RLSpec) -> CircuitGenome:
    ...
    for ep in range(spec.episodes):
        ...
        for _ in range(spec.max_steps):
            ...
            rb.add(x, a, r, x2, done, u_ep.detach()[a], u_al.detach()[a], unc_ratio.detach())
            ...
            if len(rb) >= spec.warmup_steps and (global_step % spec.train_every) == 0:
                ...
                with torch.no_grad():
                    ...
                    for i in range(s2.shape[0]):
                        ...
                        m, n, a, b = torch.split(evi, int(spec.n_actions), dim=-1)
                ...
                loss_evidence = NIG_NLL(target, mu_t, v_t, alpha_t, beta_t) + \
                ...
                '''
                lambda_1 = 0.7
                new_priorities = lambda_1 * curr_unc_ratios + (1 - lambda_1) * torch.tensor(unc_ratios)
                # print(f"curr_unc_ratios: {curr_unc_ratios}; \nnext_curr_unc_ratios: {torch.tensor(unc_ratios)}; \nnew_priorities: {new_priorities}")
