...
class Decoder(ABC):
    ...
    def from_dict(cls, serialized: dict[str, Any]) -> Decoder:
        ...
        The serialized dict is not modified, so the same dict can be
        deserialized more than once.

        Args:
            serialized: is the dict created from the serialized JSON. Its
                ``"class"`` entry names the decoder class and its ``"args"``
                entry holds the constructor arguments, plus an optional
                ``"state_dict"`` of parameter values for ``torch.nn.Module``
                decoders.

        Returns:
            A constructed Decoder object created from the given JSON. If a
            module decoder has no ``"state_dict"`` it keeps its freshly
            initialized parameters.

        Raises:
            KeyError: If ``serialized`` is missing its ``"class"`` or ``"args"``
                entry, or ``"class"`` does not name a class in this module.
        """
